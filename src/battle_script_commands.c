...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) == GetCurrentPartyLevelCap() || CheckSpeedchoiceOption(EXPMATH, EXP_NONE) == TRUE)
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
            ...
            else
            {
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    // SPEEDCHOICE CHANGE: Use the original speedchoice handling.
                    /*
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) != GetCurrentPartyLevelCap() && CheckSpeedchoiceOption(EXPMATH, EXP_NONE) == FALSE)
            {
